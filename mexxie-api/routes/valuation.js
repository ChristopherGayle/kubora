...
// GET /api/valuation — latest market valuation for a region
router.get('/', async (req, res) => {
  const region = db.esc(req.query.region || 'Worldwide');

  const sql = "SELECT * FROM prism_market_valuation " +
    "WHERE region = '" + region + "' LATEST ON ts PARTITION BY region;";
...
// GET /api/valuation/history — valuation history for a region
...
// POST /api/valuation — save market valuation data
